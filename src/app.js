    ...
    if (view === "detail") {
      ...
      const specs = [
        ...
        similar = products
          ...
          .slice(0, 3);
      const productDescription = description ? `<section class="hm-product-description"><h2>${t("aboutDevice")}</h2><p class="hp-description">${esc(description)}</p></section>` : '';
...
<section class="hp-detail-hero"><div class="hp-gallery-panel"><div class="hp-gallery-head"><span>${icon("camera")}${p.photoKind === "actual" ? t("productPhoto") : p.photoKind === "model" ? (lang === "uk" ? "Ілюстрація моделі" : "Zdjęcie modelu") : (lang === "uk" ? "Зображення товару" : "Zdjęcia produktu")}</span><span>HMG-${p.id.toString().padStart(3, "0")}</span></div>${photo(p)}${p.images.length > 1 ? `<div class="hp-toprow hp-thumbs">${p.images.map((src, i) => `<button type="button" class="hp-button" data-image="${i}"><img src="${esc(pictureUrl(src))}" alt="${i + 1}"></button>`).join("")}</div>` : ""}</div><div class="hm-product-overview"><div class="hp-kicker">${esc(p.brand)} · ${t("verifiedLabel")}</div><h1>${esc(productTitle(p))}</h1><p class="hp-detail-config">${esc(storefront.specSummary(p))}</p><div class="hp-detail-badges"><span class="hp-demo-status">${t("statuses")[p.status]}</span>${p.newArrival === "true" ? `<span class="hp-badge-new">${t("newArrival")}</span>` : ""}${p.bestseller === "true" ? `<span class="hp-badge-best">${t("bestseller")}</span>` : ""}${discountPercent(p) ? `<span class="hp-badge-sale">-${discountPercent(p)}%</span>` : ""}</div><div class="hp-stock ${qty === 1 ? "hp-stock-low" : ""}">${qty === 1 ? t("onlyOne") : `${qty} ${t("unitsLeft")}`}</div>${priceBlock(p, true)}<div class="hm-buy-facts">${p.condition ? `<span><small>${t("conditionShort")}</small><b>${esc(localizedValue(p.condition))}</b></span>` : ""}${p.warranty ? `<span><small>${t("warrantyShort")}</small><b>${esc(localizedValue(p.warranty))}</b></span>` : ""}</div>${purposeCodes.length ? `<div class="hp-purpose-tags">${purposeCodes.map((code) => `<span>${t(purposeKey(code))}</span>`).join("")}</div>` : ""}${productDescription}${benefitCodes.length ? `<div class="hp-benefit-list">${benefitCodes.map(code=>`<span>${icon('check-circle-2')}${t(benefitKey(code))}</span>`).join('')}</div>`:''}</div></section>
<section class="hp-detail-lower"><details class="hp-detail-info-card hm-spec-details"><summary>${t('specTitle')}</summary><div class="hp-detailspec">${specs.map(([k,v])=>`<div><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}</div></details></section>
<section class="hm-product-review"><div><span class="hp-kicker">${lang==='uk'?'ПОДИВИСЬ БЛИЖЧЕ':'ZOBACZ Z BLISKA'}</span><h2>${lang==='uk'?'Більше фото та огляд у Telegram':'Więcej zdjęć i prezentacja w Telegramie'}</h2><p>${lang==='uk'?'Переглянь матеріали про товар, перш ніж обирати комплектацію.':'Poznaj produkt, zanim wybierzesz konfigurację.'}</p></div><a class="hp-button hp-channel-button" href="${esc(telegramPostUrl(p.telegramPost))}" target="_blank" rel="noopener noreferrer">${icon('play-circle')}${lang==='uk'?'Відкрити фото й огляд':'Otwórz zdjęcia i prezentację'}${icon('arrow-up-right')}</a></section>
<section id="hm-product-checkout" class="hm-product-checkout hp-buy-panel" tabindex="-1"><div class="hm-checkout-heading"><span class="hp-kicker">${lang==='uk'?'ТВІЙ КОМПЛЕКТ':'TWÓJ ZESTAW'}</span><h2>${lang==='uk'?'Обери доповнення та замовляй':'Wybierz dodatki i zamów'}</h2><p>${lang==='uk'?'Залиш базову комплектацію або додай доступні опції. Сума оновиться одразу.':'Zostaw podstawową konfigurację lub wybierz dostępne opcje. Suma aktualizuje się od razu.'}</p></div>
...
<div class="hm-checkout-actions"><div class="hp-detail-actions hp-detail-primary-actions"><button type="button" class="hp-button hp-primary" data-order-one="${p.id}" ${p.status !== 0 || qty < 1 ? "disabled" : ""}>${icon("send")}${lang === "uk" ? "Замовити в Telegram" : "Zamów w Telegramie"}</button></div><div class="hp-detail-secondary-actions"><button type="button" class="hp-button hp-choice-button" data-add="${p.id}" ${p.status !== 0 || qty < 1 ? "disabled" : ""}>${icon(cart.includes(p.id) ? "check" : "shopping-bag")}${cart.includes(p.id) ? t("added") : t("add")}</button><button type="button" class="hp-button" data-compare="${p.id}">${icon(compare.includes(p.id) ? "check" : "columns-2")}${t("compare")}</button><button type="button" class="hp-button" id="hp-share" aria-label="${t("share")}">${icon("share-2")}</button></div><p class="hp-muted hp-small">${lang==='uk'?'У Telegram відкриється повідомлення з вибраною комплектацією та сумою. Залишиться надіслати його нам.':'W Telegramie otworzy się wiadomość z wybraną konfiguracją i sumą. Wystarczy ją wysłać.'}</p></div></section>${reviewsBlock(3)}${similar.length ? `<section class="hp-home-section hp-similar-section"><div class="hp-section-title"><h2>${t("similar")}</h2></div><div class="hp-grid">${similar.map(productCard).join("")}</div></section>` : ""}</div>`;
      ...
      content.insertAdjacentHTML("beforeend", `<div class="hp-mobile-order-bar"><div><small>${esc(productTitle(p))}</small><strong>${money(selectedConfig(p).total)} zł</strong></div><button type="button" class="hp-button hp-primary" data-jump-config>${lang==='uk'?'До комплектації':'Konfiguracja'}</button><a href="${esc(telegramPostUrl(p.telegramPost))}" target="_blank" rel="noopener noreferrer" data-track-target="telegram_product" aria-label="${t("telegramMedia")}">${icon("send")}<span>Telegram</span></a></div>`);
    }
    if (view === "compare") {
