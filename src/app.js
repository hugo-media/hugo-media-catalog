...
<section class="hp-detail-hero hm-compact-detail" data-detail-product="${p.id}"><header class="hm-detail-heading"><div class="hp-kicker">${esc(p.brand)} · ${t("verifiedLabel")}</div><h1>${esc(productTitle(p))}</h1><p class="hp-detail-config">${esc(storefront.specSummary(p))}</p></header><div class="hp-gallery-panel"><div class="hp-gallery-head"><span>${icon("camera")}${p.photoKind === "actual" ? t("productPhoto") : p.photoKind === "model" ? (lang === "uk" ? "Ілюстрація моделі" : "Zdjęcie modelu") : (lang === "uk" ? "Зображення товару" : "Zdjęcia produktu")}</span><span>HMG-${p.id.toString().padStart(3, "0")}</span></div>${photo(p)}${p.images.length > 1 ? `<div class="hp-toprow hp-thumbs">${p.images.map((src, i) => `<button type="button" class="hp-button" data-image="${i}"><img src="${esc(pictureUrl(src))}" alt="${i + 1}"></button>`).join("")}</div>` : ""}</div><div class="hm-product-overview"><div class="hp-detail-badges"><span class="hp-demo-status">${t("statuses")[p.status]}</span>${p.newArrival === "true" ? `<span class="hp-badge-new">${t("newArrival")}</span>` : ""}${p.bestseller === "true" ? `<span class="hp-badge-best">${t("bestseller")}</span>` : ""}${discountPercent(p) ? `<span class="hp-badge-sale">-${discountPercent(p)}%</span>` : ""}</div><div class="hp-stock ${qty === 1 ? "hp-stock-low" : ""}">${qty === 1 ? t("onlyOne") : `${qty} ${t("unitsLeft")}`}</div>${priceBlock(p, true)}<div class="hm-buy-facts">${p.condition ? `<span><small>${t("conditionShort")}</small><b>${esc(localizedValue(p.condition))}</b></span>` : ""}${p.warranty ? `<span><small>${t("warrantyShort")}</small><b>${esc(localizedValue(p.warranty))}</b></span>` : ""}</div>${purposeCodes.length ? `<div class="hp-purpose-tags">${purposeCodes.map((code) => `<span>${t(purposeKey(code))}</span>`).join("")}</div>` : ""}${productDescription}${benefitCodes.length ? `<div class="hp-benefit-list">${benefitCodes.map(code=>`<span>${icon('check-circle-2')}${t(benefitKey(code))}</span>`).join('')}</div>`:''}</div>

<section class="hm-product-review"><a class="hp-button hp-channel-button" href="${esc(telegramPostUrl(p.telegramPost))}" target="_blank" rel="noopener noreferrer">${icon('play-circle')}<span>${p.telegramPost ? (lang==='uk'?'Фото та відеоогляд у Telegram':'Zdjęcia i prezentacja w Telegramie') : (lang==='uk'?'Більше в Telegram-каналі':'Więcej na kanale Telegram')}</span>${icon('arrow-up-right')}</a></section>
<section id="hm-product-checkout" class="hm-product-checkout hp-buy-panel" tabindex="-1"><div class="hm-checkout-heading"><h2>${lang==='uk'?'Комплектація та замовлення':'Konfiguracja i zamówienie'}</h2></div>
...
<div class="hm-checkout-actions"><div class="hp-detail-actions hp-detail-primary-actions"><button type="button" class="hp-button hp-primary" data-order-one="${p.id}" ${p.status !== 0 || qty < 1 ? "disabled" : ""}>${icon("send")}${lang === "uk" ? "Замовити в Telegram" : "Zamów w Telegramie"}</button></div><div class="hp-detail-secondary-actions"><button type="button" class="hp-button hp-choice-button" data-add="${p.id}" ${p.status !== 0 || qty < 1 ? "disabled" : ""}>${icon(cart.includes(p.id) ? "check" : "shopping-bag")}${cart.includes(p.id) ? t("added") : t("add")}</button><button type="button" class="hp-button" data-compare="${p.id}">${icon(compare.includes(p.id) ? "check" : "columns-2")}${t("compare")}</button><button type="button" class="hp-button" id="hp-share" aria-label="${t("share")}">${icon("share-2")}</button></div><p class="hp-muted hp-small">${lang==='uk'?'Комплектація й сума потраплять у повідомлення Telegram.':'Konfiguracja i suma trafią do wiadomości w Telegramie.'}</p></div></section></section><section class="hp-detail-lower"><details id="hm-full-specs" class="hp-detail-info-card hm-spec-details"><summary>${t('specTitle')}</summary><div class="hp-detailspec">${specs.map(([k,v])=>`<div><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}</div></details></section>${reviewsBlock(3)}${similar.length ? `<section class="hp-home-section hp-similar-section"><div class="hp-section-title"><h2>${t("similar")}</h2></div><div class="hp-grid">${similar.map(productCard).join("")}</div></section>` : ""}</div>`;
