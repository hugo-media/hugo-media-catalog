...
export function createStorefront(c) {
  ...
  const E = c.esc;
  const specSummary = p => (p.cat===3 ? [p.noise, p.connection] : p.cat===5 ? [specLabel('screen',p.screen),p.resolution,p.refreshRate] : [p.cpu,p.ram && `${specLabel('ram',p.ram)} RAM`,p.ssd && `${specLabel('ssd',p.ssd)}${p.cat===0?' SSD':''}`, p.cat===0 && specLabel('screen',p.screen)]).filter(Boolean).join(' · ');
  function card(p) {
    ...
    return `<article class="hp-product"><button class="hp-product-open" type="button" data-detail="${p.id}" aria-label="${E(c.t('detail'))}: ${E(p.name)}"><div class="hp-card-badges">${tags.join('')}</div>${c.photo(p)}<div class="hp-product-body"><div class="hp-product-meta"><span>${E(p.brand)}</span>${purpose?`<span>${E(c.t(c.purposeKey(purpose)))}</span>`:''}</div><h3 class="hp-product-name">${E(c.productTitle(p))}</h3><p class="hp-specs">${E(specSummary(p))}</p><div class="hm-card-facts">${p.condition?`<span>${E(c.localizedValue(p.condition))}</span>`:''}${p.warranty?`<span>${c.icon('shield-check')}${E(c.localizedValue(p.warranty))}</span>`:''}</div>${extras.length?`<div class="hm-card-extras">${extras.map(E).join(' · ')}</div>`:''}</div></button><div class="hp-product-foot"><div class="hp-price-row">${c.priceBlock(p)}<button type="button" class="hp-compare-add" data-compare="${p.id}" aria-label="${E(c.t('compare'))}: ${E(p.name)}" aria-pressed="${c.compare.includes(p.id)}">${c.icon(c.compare.includes(p.id)?'check':'columns-2')}</button></div><div class="hp-card-cta"><button type="button" class="hp-button hp-card-details" data-detail="${p.id}">${c.t('cardDetails')}</button><button type="button" class="hp-button hp-primary hp-card-order" data-order-one="${p.id}">${c.icon('send')}${c.t('cardOrder')}</button></div></div></article>`;
  }
  function home(products, picks, tab) {
    const available=products.filter(p=>p.status===0&&c.stockQty(p)>0);
    const featured=available.find(p=>p.cat===0&&p.bestseller==='true'&&p.images.length&&!picks.some(pick=>pick.id===p.id))||available.find(p=>p.cat===0&&p.images.length&&!picks.some(pick=>pick.id===p.id))||available.find(p=>p.cat===0&&p.images.length);
