...
export async function connect() {
  ...
  if (!client) {
    const { createClient } =
      await import("https://esm.sh/@supabase/supabase-js@2.57.4");
    client = createClient(
      window.HUGO_CONFIG.supabaseUrl,
      window.HUGO_CONFIG.supabaseKey,
      {
        auth: {
          persistSession: true,
          autoRefreshToken: true,
          detectSessionInUrl: true,
          storageKey: "hmg-catalog-auth",
        },
      },
    );
  }
...
export async function listProducts() {
  ...
  for (let start = 0; ; start += 500) {
    ...
    products.push(...data.map(fromRow));
...
export function photoUrl(path) {
  if (!path) return "";
  const url = window.HUGO_CONFIG?.supabaseUrl;
  if (!url) return "";
...
export async function compressPhoto(file) {
  ...
  try {
    if (bitmap.width * bitmap.height > 60000000) throw Error("imageError");
    ...
    return await new Promise((resolve, reject) =>
      canvas.toBlob(
        (blob) => (blob ? resolve(blob) : reject(Error("imageError"))),
        "image/webp",
        0.84,
      ),
    );
...
export async function saveProduct(p, pictures) {
  ...
  try {
    ...
    const values = toRow({ ...p, images: paths });
    writingRow = true;
    let request = p.id
    ...
    if (staged.length && !writingRow)
      await c.storage.from(BUCKET).remove(staged);
    throw e;
  }
  return fromRow(row);
}
...
export async function trackEvent(event) {
  if (!ready) return;
  const c = await connect();
  const { error } = await c.from("hmg_catalog_events").insert(event);
  if (error) throw error;
}
...
export async function saveReview(review, picture) {
  ...
  if (userError || !user) throw Error("notAdmin");
  let imagePath = typeof picture === "string" ? picture : "";
  let staged = "";
  try {
    if (picture && typeof picture !== "string") {
      staged = `${user.id}/${crypto.randomUUID()}.webp`;
      const { error } = await c.storage
        .from(BUCKET)
        .upload(staged, picture.blob, {
          contentType: "image/webp",
          upsert: false,
          cacheControl: "3600",
        });
    ...
    const request = review.id
      ? c.from("hmg_catalog_reviews").update(values).eq("id", review.id)
      : c.from("hmg_catalog_reviews").insert(values);
    const { data, error } = await request.select().single();
    if (error) throw error;
    return data;
  } catch (error) {
    if (staged) await c.storage.from(BUCKET).remove([staged]);
    throw error;
  }
}
